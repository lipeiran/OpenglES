...
GLuint cube_Texture,floor_Texture,quad_Texture,cubeVAO,cubeVBO,planeVAO,planeVBO,quadVAO,quadVBO,framebuffer,rbo,pingpongFBO[2],pingpongColorbuffers[2];
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    ...
    for (unsigned int i = 0; i < amount; i++)
    {
        ...
        self.screenShader->setInt("horizontal", horizontal);
        glBindTexture(GL_TEXTURE_2D, first_iteration ? quad_Texture : pingpongColorbuffers[!horizontal]);  // bind texture of other framebuffer (or scene if first iteration)
        glBindVertexArrayOES(quadVAO);
