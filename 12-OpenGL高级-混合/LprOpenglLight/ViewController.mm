...
GLuint cube_Texture,floor_Texture,grass_Texture,cubeVAO,cubeVBO,planeVAO,planeVBO,transparentVAO,transparentVBO;
...
- (void)_initVAO
{
    ...
    glBindVertexArrayOES(0);
    // transparent VAO
    glGenVertexArraysOES(1, &transparentVAO);
    glGenBuffers(1, &transparentVBO);
    glBindVertexArrayOES(transparentVAO);
...
- (void)_initTextures
{
    ...
    NSString *grass_path = [[NSBundle mainBundle]pathForResource:@"window" ofType:@"png"];

    cube_Texture = loadTexture([cube_path UTF8String]);
    floor_Texture = loadTexture([floor_path UTF8String]);
    grass_Texture = loadTexture([grass_path UTF8String]);
}
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    ...
    {
        float distance = glm::length(viewPos - windows[i]);
        sorted[distance] = windows[i];
    }
    for (std::map<float, glm::vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it)
    {
        modelT = glm::mat4();
        modelT = glm::translate(modelT, it->second);
        self.ourShader->setMat4("model", modelT);
        glDrawArrays(GL_TRIANGLES, 0, 6);
    }
}
