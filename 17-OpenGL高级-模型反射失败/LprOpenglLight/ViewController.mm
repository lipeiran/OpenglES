...
glm::vec3 viewPos(_initRadius*cos(_initAngle),2.0f,_initRadius*sin(_initAngle));

GLint pos_Location,texNormal_Location,sb_Pos_Location;
...
- (void)_initShader
{
    ...
    texNormal_Location = glGetAttribLocation(self.ourShader->ID, "aNormal");
...
- (void)_initVAO
{
    ...
    glEnableVertexAttribArray(texNormal_Location);
    glVertexAttribPointer(texNormal_Location, 3, GL_FLOAT, GL_FALSE, 6*sizeof(float), (void*)(3*sizeof(float)));
...
{
    self.ourShader->use();
    self.ourShader->setInt("skybox", 0);
    self.sbShader->use();
...
{
    viewPos = glm::vec3(_initRadius*cos(_initAngle),4.0f,_initRadius*sin(_initAngle));
    _initAngle += 0.01f;
    // render
